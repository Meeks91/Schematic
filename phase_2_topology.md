...
**Editable artifact (interactive-first):** author the roster as a Mermaid draft — one subgraph per feature, vertically stacked (chain subgraphs with invisible `~~~` links), classes as boxes, storage as cylinders, classDefs for locked/new/mod/storage/fork-open/back-ref (**dark-mode safe, BINDING**: the editor is dark-themed and its label colour overrides `color:` — use dark fills + white text + status colour on the stroke, e.g. `classDef new fill:#14532d,stroke:#4ade80,color:#fff`; never pastel/white fills; **never nest `"` inside a `["…"]` or `|"…"|` label**).

**Present the topology to the user through the CLI — never by hand (BINDING).** After authoring the draft, run:
```
schematic roster present 2 --schematic <name> --file <draft.mmd>
```
This is the ONE legitimate way to open the roster gate. In one step it: validates the draft (invalid → nothing launched, nothing written, exit 1 — fix and re-run); writes canonical `roster.mmd` **only on a passing validation** (so the file's existence proves it was presented); launches the visual editor detached; and prints a **nonce-stamped envelope**. **The gate you show the user IS that envelope** — paste its fenced lines verbatim (the nonce is the proof the launcher ran) and author the roster strips between the `EDITOR LIVE` block and the closing fence. A roster gate shown WITHOUT the stamped envelope is a skipped launch and a phase violation. If the command exits non-zero it launched nothing and printed no envelope — you have no gate; fix the cause, never hand-author one.
