...
**FIRST action in Phase 2 — launch the blank editor through the CLI (BINDING). Before authoring a single node, run:**
```
schematic roster init 2 --schematic <name>
```
This is the ONE legitimate way to open the roster gate, and it runs on Phase-2 entry — nothing you author precedes it. In one step it: scaffolds a **blank skeleton `roster.mmd`** from the Phase-1 features (one placeholder subgraph per feature, dark-mode classDefs preset) if none exists yet — re-entry re-launches the existing file, never clobbering your filled canvas; validates it (invalid → nothing launched, exit 1); launches the visual editor detached; records the launch in state; and prints a **nonce-stamped envelope**. **The gate you show the user IS that envelope** — paste its fenced lines verbatim (the nonce is the proof the launcher ran). You then FILL the live editor canvas. **Do NOT post the topology (strips, node list, ASCII) in chat — the editor canvas is the only roster surface; the stamped envelope is the entire chat gate.** A roster gate shown WITHOUT the stamped envelope is a skipped launch and a phase violation. **`schematic phase sign-off 2` refuses until the launch is recorded** — a chat-only roster cannot lock the phase.

Node conventions once you fill it: one subgraph per feature, vertically stacked (chain subgraphs with invisible `~~~` links), classes as boxes, storage as cylinders, classDefs for locked/new/mod/storage/fork-open/back-ref (**dark-mode safe, BINDING**: the editor is dark-themed and its label colour overrides `color:` — use dark fills + white text + status colour on the stroke, e.g. `classDef new fill:#14532d,stroke:#4ade80,color:#fff`; never pastel/white fills; **never nest `"` inside a `["…"]` or `|"…"|` label**).

**Arm the Q&A watcher (BINDING).** The envelope prints the exact `watcher.py` command — run it backgrounded via your own shell (background tasks notify the agent on process exit, which the CLI cannot do for you); re-arm after each answer. An editor without a watcher is dead air.
